...
export default function (hubUrl: string) {
  return [
    rest.post(`${hubUrl}/negotiate`, async (req, res, ctx) => {
      ...
      if (version !== 0 && version !== 1) return res(ctx.status(400));
    ...
    rest.get(hubUrl, async (req, res, ctx) => {
      ...
      if (connection === null) return res(ctx.status(error));
    ...
    rest.post(hubUrl, async (req, res, ctx) => {
      ...
      if (connection.needsHandshake) {
        ...
        if (request.protocol === "json" && request.version === 1) {
          ...
          return res(
            ctx.body(signalRify({ error: "Requested protocol not supported" }))
          );
      }
